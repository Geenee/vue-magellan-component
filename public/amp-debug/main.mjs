      ...
      if (allowedExecution && allowedMutation) {
        ...
        if (target) {
          ...
          workerContext.messageToWorker({
            ...
            [38
            /* data */
            ]: offscreen // Object, an OffscreenCanvas
...
const ScrollIntoViewProcessor = (strings, nodes, workerContext, objectContext, config) => {
  const allowedExecution = config.executorsAllowed.includes(14
  /* SCROLL_INTO_VIEW */
  );
  return {
    execute(mutations, startPosition, allowedMutation) {
...
class MutatorProcessor {
  ...
  constructor(stringContext, nodeContext, workerContext, config, objectContext) {
    ...
    this.executors = {
      ...
      [14
      /* SCROLL_INTO_VIEW */
      ]: ScrollIntoViewProcessor.apply(null, args)
    };
  }
  /**
...
const isGetStorage = message => message[12
/* type */
] === 11
/* GET_STORAGE */
;
...
class WorkerContext {
  ...
  constructor(baseElement, nodeContext, workerDOMScript, authorScript, config) {
    ...
    const code = `'use strict';(function(){${workerDOMScript}self['window']=self;var workerDOM=WorkerThread.workerDOM;WorkerThread.hydrate(workerDOM.document,${JSON.stringify(strings)},${JSON.stringify(skeleton)},${JSON.stringify(cssKeys)},${JSON.stringify(globalEventHandlerKeys)},[${window.innerWidth},${window.innerHeight}],${JSON.stringify(localStorageInit)},${JSON.stringify(sessionStorageInit)});workerDOM.document[${59
    /* observe */
    }](this);Object.assign(self,workerDOM);}).call(self);${authorScript}//# sourceURL=${encodeURI(config.authorURL)}`;
    ...
    if (!config.sandbox) {
      ...
      this[55
      /* worker */
      ] = new IframeWorker(URL.createObjectURL(new Blob([code])), config.sandbox.iframeUrl);
    }
